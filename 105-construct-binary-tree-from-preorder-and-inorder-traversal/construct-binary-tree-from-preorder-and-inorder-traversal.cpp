...
public:
    ...
    TreeNode* construct(vector<int>& preorder, int preStart, int preEnd, vector<int>& inorder, int inStart, int inEnd, map<int,int> &mp) {
        ...
        node->left = construct(preorder, preStart+1, preStart + numsLeft, inorder, inStart, inRoot - 1, mp);
        node->right = construct(preorder, preStart + numsLeft + 1, preEnd, inorder, inRoot + 1, inEnd, mp);
        return node;
    }
};